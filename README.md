...
- [ ] Explain benefit(s) using `client-side routing`?
  ...
  > Answer: Page navigation is quicker, smoother, no need for a brand new refresh and no need to fetch the data all over again every time you click something else on the Webpage. In a nutshell its a much more enjoyable user experience because of minimal reload times and smoother navigation.

- [ ] What does AJAX stand for?

  > Answer: asynchronous javascript and xml. AJAX’s core function is to update web content without needing to reload an entire web page when only a small portion of content on the page needs to change.

- [ ] What are `controlled components` in React?

  > Answer: A controlled component takes its real-time value through props and notifies changes through callbacks like "onChange". A parent component handles the callback and manages its own state by passing the new values as props to the controlled component. As opposed to uncontrolled components, which store their own state internally.

- [ ] Name three tools/libraries for making AJAX requests.
  > Answer: Axios, jQuery AJAX, and window.fetch.
